"""
GraphAdapter Ensemble — Majority-Vote Evaluation from Single-Backbone Runs.

Reuses trained models from output/graphadapter_experiments/ (produced by
run_graphadapter_multiseed.py).  For each (dataset, shots, seed) combination
where all 3 backbones completed, loads each checkpoint, runs inference on the
test set, and computes the majority-vote ensemble.

Metrics are identical to the HeGraphAdapter ensemble (ensemble_train_eval.py):
accuracy, macro_f1, weighted_f1, kappa, macro_auc — per backbone and ensemble.

Usage
-----
# Evaluate everything discovered under the default experiments dir:
python evaluate_graphadapter_ensemble.py

# Restrict to a specific dataset / shot count:
python evaluate_graphadapter_ensemble.py --datasets lunghist --shots 4

# Point at a different experiments dir:
...
# ── Config maps (must match run_graphadapter_multiseed.py) ────────────────────
...
BACKBONE_CONFIGS = {
    "biomedclip": "configs/trainers/GraphCLIP_v2/biomedclip_paper.yaml",
    "plip":       "configs/trainers/GraphCLIP_v2/plip_paper.yaml",
    "conch":      "configs/trainers/GraphCLIP_v2/conch_paper.yaml",
}
...
def discover_runs(experiments_dir, datasets, shots, seeds):
    ...
    # Keep only groups with all 3 backbones
    complete = {
        k: v for k, v in groups.items() if len(v) == len(BACKBONES)
    }
    return complete
...
def evaluate_backbone(model_dir, dataset_cfg, backbone_cfg, num_shots, seed, root):
    ...
    for batch in tqdm(trainer.test_loader, desc=f"    inference", leave=False):
...
def main():
    parser = argparse.ArgumentParser(
        description="GraphAdapter ensemble (majority vote) from single-backbone runs")
    parser.add_argument("--experiments-dir", type=str,
                        default="output/graphadapter_experiments")
    parser.add_argument("--output-dir", type=str,
                        default="output/graphadapter_ensemble")
    ...
    # Load already-evaluated ensembles for resume
    ...
    for idx, ((dataset, shots, seed), backbone_dirs) in enumerate(sorted(groups.items()), 1):
        ...
        # Resume: skip if already in JSONL
        if key in done:
            skipped += 1
